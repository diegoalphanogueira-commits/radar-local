(() => {
  ...
  function feedNode() { return document.querySelector('[role="feed"]'); }
  ...
  function nudgeScroller(scroller, feed) {
    ...
    try { scroller.dispatchEvent(new WheelEvent("wheel", { deltaY: 1500, bubbles: true, cancelable: true })); } catch {}
  }

  async function scanAndScroll(maxScrolls = 55) {
    const feed = await waitForFeed();
    if (!feed) {
      const leads = scan(document);
      return { leads, metrics: { unique: leads.length, rounds: 0, feedFound: false, stopReason: "no-feed" } };
    }

    const scroller = findScroller(feed);
    const all = new Map();
    const rounds = Math.max(24, Math.min(Number(maxScrolls) || 55, 70));
    let stableRounds = 0;
    let motionlessRounds = 0;
    let endRounds = 0;
    let executed = 0;
    let stopReason = "round-limit";
    let lastGrowthRound = 0;
    ...
    try {
      ...
      for (let i = 0; i < rounds; i += 1) {
        ...
        const step = Math.max(1100, Math.round(viewport * (i % 3 === 0 ? 2.0 : 1.45)));
        ...
        await sleep(600 + Math.round(Math.random() * 350));
        addScanned(all, feed);
        ...
        if (!grew && (i + 1) % 3 === 0) {
          ...
          nudgeScroller(scroller, feed);
          await sleep(700 + Math.round(Math.random() * 250));
          addScanned(all, feed);
        }

        if (endRounds >= 1 && stableRounds >= 3) {
          stopReason = "end-detected";
          break;
        }
        if (stableRounds >= 7 && motionlessRounds >= 3) {
          stopReason = "stable";
          break;
        }
        if (executed >= 18 && stableRounds >= 5 && executed - lastGrowthRound >= 5) {
          stopReason = "no-new-results";
          break;
        }
      }
      ...
      for (let probe = 0; probe < 2; probe += 1) {
        ...
        nudgeScroller(scroller, feed);
        await sleep(650 + probe * 200);
        addScanned(all, feed);
        if (all.size === before && probe === 0 && reachedEnd(feed)) break;
      }
    } finally {
      observer.disconnect();
    }
  ...
  function extractDetail() {
    ...
    const coords = detailCoords(location.href);
    return {
      name,
      address,
      phone,
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message?.cmd === "SCAN_SCROLL") {
      scanAndScroll(Number(message.maxScrolls) || 55)
