(() => {
  ...
  const MEMORY_KEY = "radarV1BestSearchMemoryV4";
  ...
  const GROUPS = [
    ...
    { id:"odontologia", test:/odont|dentist|dental|ortodont|implantodont|endodont/, terms:["dentista","clínica odontológica","odontologia","consultório odontológico","cirurgião-dentista","ortodontista","implantodontista","odontopediatra"] },
  ...
  function variants(term, region) {
    ...
    return [...new Set(list.map(v => `${v} ${region}`.trim()))].slice(0, 10);
  }
  ...
  function exactSeed(ctx) {
    const memory = loadJson(MEMORY_KEY, {});
    return dedupe(memory[contextKey(ctx)]?.leads || []);
  }
  function saveMemory(ctx, rows) {
    ...
    const kept = Object.entries(memory).sort((a,b) => Number(b[1]?.updatedAt || 0) - Number(a[1]?.updatedAt || 0)).slice(0, 30);
  ...
  function ensureCoverageControl() {
    ...
    box.innerHTML = `<label for="radarCoverageTarget">Cobertura</label><select id="radarCoverageTarget"><option value="30">30+ empresas</option><option value="50">50+ empresas</option><option value="100">100+ empresas</option><option value="max">Máxima</option></select><small>Quanto maior, mais o Radar insiste.</small>`;
  ...
  function write(ctx, rows, patch = {}) {
    ...
    localStorage.setItem(META_KEY, JSON.stringify({ ...previous, term: ctx.term, region: ctx.region, radiusKm: ctx.radiusKm, activeSearchSegment: ctx.segment, activeSearchSession: ctx.sessionId, coverageTarget: ctx.targetMode, count: clean.length, ...patch }));
    window.dispatchEvent(new CustomEvent("radar:maps-data-updated", { detail: { count: clean.length, source: "radar-rc15" } }));
  }

  async function syncPartial(ctx) {
    if (!activeSession || activeSession.id !== ctx.sessionId) return [];
    try {
      const response = await chrome.runtime.sendMessage({ cmd: "GET_STATE" });
      if (!activeSession || activeSession.id !== ctx.sessionId) return [];
      const rows = dedupe(response?.leads || []);
      if (rows.length) write(ctx, rows, { searchInProgress: true, searchHeartbeatAt: new Date().toISOString() });
      return rows;
    } catch { return []; }
  }
  ...
  async function startSearch() {
    ...
    try {
      ...
      await cancelCurrent();
      const ctx = { ...base, sessionId: `radar-${Date.now()}-${Math.random().toString(36).slice(2,8)}` };
      activeSession = { id: ctx.sessionId, phase: "discovery", ctx };
      const seed = exactSeed(ctx);
      buttonsBusy(true);
      status("Mapeando região...", `${ctx.segment} · cobertura ${ctx.targetMode === "max" ? "Máxima" : `${ctx.targetMode}+`} · 2 buscas em paralelo.`);
      write(ctx, seed, { searchInProgress: true, searchStartedAt: new Date().toISOString(), v4Center: null });

      const result = await chrome.runtime.sendMessage({
        cmd: "RUN_MARKET_SEARCH_V4",
        sessionId: ctx.sessionId,
        term: ctx.term,
        segment: ctx.segment,
        region: ctx.region,
        radiusKm: ctx.radiusKm,
        queries: variants(ctx.term, ctx.region),
      ...
      if (!result?.ok) {
        ...
        throw new Error(result?.error || "Falha na descoberta.");
      }

      let rows = saveMemory(ctx, dedupe(result.leads || []));
      write(ctx, rows, { searchInProgress: true, discoveryCompletedAt: new Date().toISOString(), coverageSearchesDone: result.searchesDone || 0, v4Center: result?.context?.center ? { ...result.context.center, region: ctx.region } : null });
      activeSession.phase = "enrich";
      status("Empresas encontradas", `${rows.length} negócios consolidados. Agora abrindo 3 fichas em paralelo para telefone e site.`);

      const enriched = await chrome.runtime.sendMessage({ cmd: "ENRICH_ALL", sessionId: ctx.sessionId, limit: Math.min(120, Math.max(50, rows.length)) });
      if (!activeSession || activeSession.id !== ctx.sessionId) return;
      if (enriched?.ok && Array.isArray(enriched.leads)) rows = saveMemory(ctx, dedupe(enriched.leads));
      const phones = rows.filter(lead => digits(lead.phone).length >= 10).length;
      const sites = rows.filter(lead => String(lead.website || "").trim()).length;
      write(ctx, rows, { searchInProgress: false, searchCompletedAt: new Date().toISOString(), phoneCount: phones, siteCount: sites, v4Center: result?.context?.center ? { ...result.context.center, region: ctx.region } : null });
      status("Mapeamento concluído", `${rows.length} negócios · ${phones} com telefone · ${sites} com site.`, true);
      activeSession = null;
    } catch (error) {
      if (activeSession) {
        const ctx = activeSession.ctx;
        const partial = await syncPartial(ctx);
        const count = partial.length;
        status(count ? "Coleta parcial preservada" : "A coleta foi interrompida", count ? `${count} negócios já foram preservados.` : (error?.message || "Tente novamente."));
  ...
  window.addEventListener("message", event => {
    ...
    if (message.type === "GET_STATE") {
      chrome.runtime.sendMessage({ cmd: "GET_STATE" }).then(response => post("STATE_RESULT", { requestId: message.requestId || "", response })).catch(error => post("STATE_RESULT", { requestId: message.requestId || "", response: { ok:false, error:error.message } }));
      return;
    }
    if (message.type === "ENRICH") {
      const sessionId = activeSession?.id || `manual-${Date.now()}`;
      chrome.runtime.sendMessage({ cmd: "ENRICH_ALL", sessionId, limit: Number(message.limit) || 60 }).then(response => post("ENRICH_RESULT", { requestId: message.requestId || "", response })).catch(error => post("ENRICH_RESULT", { requestId: message.requestId || "", response:{ ok:false,error:error.message } }));
      return;
    }
    if (message.type === "SITE_ENRICH") {
      chrome.runtime.sendMessage({ cmd: "SITE_ENRICH_ONE", website: String(message.website || "").trim(), force: !!message.force }).then(response => post("SITE_ENRICH_RESULT", { requestId: message.requestId || "", response })).catch(error => post("SITE_ENRICH_RESULT", { requestId: message.requestId || "", response:{ ok:false,error:error.message } }));
      return;
    }
    if (message.type === "CLEAR") {
      cancelCurrent().then(() => chrome.runtime.sendMessage({ cmd:"CLEAR" })).then(response => post("CLEAR_RESULT", { requestId:message.requestId || "", response })).catch(error => post("CLEAR_RESULT", { requestId:message.requestId || "", response:{ ok:false,error:error.message } }));
    }
  });
  ...
  chrome.runtime.onMessage.addListener(message => {
    ...
    if (message.event === "MARKET_PARTIAL") {
      status(`Varredura ${message.current || 0}/${message.total || 0}`, `${message.accumulated || 0} empresas · ${message.query || ctx.term} · ${message.area || "região"}`);
      syncPartial(ctx);
      return;
    }
    if (message.event === "SEARCH_PROGRESS" && message.stage === "opening") {
      status(`Abrindo busca ${message.current || 0}/${message.total || 0}`, `${message.query || ctx.term} · ${message.area || "região"}`);
      return;
    }
    if (message.event === "ENRICH_PROGRESS") {
      status(`Completando ${message.current || 0}/${message.total || 0}`, "3 fichas em paralelo · telefone, site e horário.");
      syncPartial(ctx);
    }
  });
