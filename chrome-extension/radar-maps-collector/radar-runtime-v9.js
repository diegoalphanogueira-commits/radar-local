(() => {
  ...
  const PAGE_LEADS_KEY = "radarMapsImportedLeadsV2";
  const PAGE_META_KEY = "radarMapsImportedMetaV2";
  let activeSessionId = "";
  ...
  function persistSnapshot(leads, ctx, extra = {}) {
    try {
      if (!Array.isArray(leads)) return false;
      localStorage.setItem(PAGE_LEADS_KEY, JSON.stringify(leads));
      const previous = JSON.parse(localStorage.getItem(PAGE_META_KEY) || "{}") || {};
      localStorage.setItem(PAGE_META_KEY, JSON.stringify({
        ...previous,
        source: "radar-rc26",
        term: ctx.term,
        region: ctx.region,
      ...
      console.warn("[Radar RC26] não foi possível sincronizar o snapshot no painel", error);
      return false;
    }
  }
  ...
  function startEnrichmentInBackground({ sessionId, discovered, discovery, ctx }) {
    const inRadius = Number(discovery?.geoStats?.inRadius || 0);
    const limit = Math.min(140, Math.max(1, inRadius || discovered.length || 1));
    ...
    chrome.runtime.sendMessage({
      ...
      if (enrichment?.ok && Array.isArray(enrichment.leads)) {
        const persisted = persistSnapshot(enrichment.leads, ctx, {
          source: "radar-rc26-enriched",
          enrichmentCompletedAt: new Date().toISOString(),
          v4Center: discovery?.context?.center ? { ...discovery.context.center, region: ctx.region } : undefined
        });

        post("PROGRESS", {
          progress: {
            event: "RC26_SYNC_COMPLETE",
            sessionId,
            accumulated: enrichment.leads.length,
            text: "Contatos atualizados no Radar."
          }
        });

        activeSessionId = "";

        // O painel V4 mantém os leads em memória. Recarregar depois de persistir
        // é a forma mais segura de exibir os dados enriquecidos sem repetir a busca.
        if (persisted) {
          setTimeout(() => {
            if (document.visibilityState !== "prerender") location.reload();
          }, 650);
        }
        return;
      }
      ...
      post("PROGRESS", {
        progress: {
          event: "RC26_SYNC_WARNING",
          sessionId,
          text: enrichment?.error || "A descoberta foi concluída, mas alguns contatos não puderam ser complementados."
        }
      });
    }).catch(error => {
      if (activeSessionId !== sessionId) return;
      ...
      post("PROGRESS", {
        progress: {
          event: "RC26_SYNC_WARNING",
          sessionId,
          text: error?.message || "Falha ao completar contatos."
        }
      });
    });
  }
  ...
  async function runBatch(message) {
    ...
    // Corrige o bug do RC25: o painel recebe a nova coleta AGORA,
    // sem esperar a etapa lenta de abrir fichas individuais.
    persistSnapshot(discovered, ctx, {
      source: "radar-rc26-discovery",
      v4Center: discovery?.context?.center ? { ...discovery.context.center, region: ctx.region } : undefined
    });

    // Telefone/site continuam sendo completados, mas em segundo plano.
    startEnrichmentInBackground({ sessionId, discovered, discovery, ctx });
