...
(() => {
  ...
  if (window.RadarV1PriorityLocal) return;
  window.RadarV1PriorityLocal = true;
  ...
  function priorityFor(lead) {
    ...
    if (!host) { score += 28; reasons.push("Sem site próprio"); }
    else if (thirdParty) { score += 20; reasons.push("Depende de link de terceiros"); }

    if (Number.isFinite(reviews)) {
      if (reviews < 20) { score += 20; reasons.push("Poucas avaliações no Google"); }
      else if (reviews < 50) { score += 14; reasons.push("Avaliações ainda podem crescer"); }
      else if (reviews < 100) score += 7;
    }

    if (Number.isFinite(rating) && rating > 0) {
      if (rating < 4.3) { score += 14; reasons.push("Reputação digital precisa de atenção"); }
      else if (rating < 4.6) score += 7;
      else if (rating >= 4.8) readiness.push("boa reputação");
    }

    if (phone.length >= 10) { score += 12; readiness.push("telefone disponível"); }
    else if (email) { score += 5; readiness.push("e-mail disponível"); }
    else score -= 8;

    if (decision) {
      if (confidence === "safe" || confidence === "matched") { score += 12; readiness.push("decisor identificado"); }
      else { score += 7; readiness.push("decisor provável"); }
    }

    if (hasSocial) { score += 4; readiness.push("presença social encontrada"); }
    ...
    return { score, level, label, summary };
  }
  ...
  function decorateCard(card, lead) {
    ...
    insight.innerHTML = `<span class="v1-priority-badge">${info.label}</span><span class="v1-priority-copy"><b>Por que abordar agora</b><em>${info.summary}</em></span><strong class="v1-priority-number">${info.score}</strong>`;
  ...
  function updateSummary(infos) {
    ...
    if (listCount) listCount.textContent = filterHighOnly ? `${counts.high} prioridades altas` : `${infos.length} empresas`;
  }

  function applyMarkerFilter() {
    const visibleIndexes = new Set([...document.querySelectorAll(".v4-lead-card")].filter(card => !card.classList.contains("v1-filter-hidden")).map(card => Number(card.dataset.cardIndex) + 1).filter(Number.isFinite));
